const actions = {
  INIT: (state, action) => {
    const penID = state.penID || 'heart'
    const pen = action.pens.find((p) => p.id === penID)
    return { ...state, penID, pens: action.pens, pen, steps: pen.steps, loading: false, working: null, error: null }
  },
